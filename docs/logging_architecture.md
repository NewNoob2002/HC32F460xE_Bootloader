...
Map-attributed RAM is 440 bytes for RTT (168-byte control block plus 272-byte RTT BSS) and 515 bytes directly for EasyLogger/port (256-byte line buffer, 248-byte state, 11-byte time buffer). The complete logging-enabled image costs 5120 additional Flash bytes and 1368 additional initialized/static RAM bytes versus the same board build without logging; this includes formatter/newlib dependencies.
